...
# From the the URL of a post with Facebook commments, recursively grabs FB comments until there are no
# more comments. It returns list a that contains two data.frames: one with parent comments, the other
# contains replies (FB comments only have two levels)
...
get.comments <- function(url = NULL, allcomments = NULL, first.time = TRUE) {
    ...
    parentcomments <- ldply(res$data, function(x) data.frame(id = x$id, fromname = x$from$name, fromid = x$from$id,
    message = x$message, created_time = x$created_time, likes = ifelse(is.null(x$likes), 0, x$likes),
    count = ifelse(is.null(x$comments$count), 0, x$comments$count)))
    
    replies <- ldply(res$data, function(x) (data.frame(ldply(x$comments$data, function(y) y$id), ldply(x$comments$data,
    function(y) y$from$name), ldply(x$comments$data, function(y) y$from$id), ldply(x$comments$data,
    function(y) y$message), ldply(x$comments$data, function(y) y$created_time))))
